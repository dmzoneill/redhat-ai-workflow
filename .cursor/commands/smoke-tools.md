...
tool_exec("gitlab_repo_view", '{"project": "automation-analytics/automation-analytics-backend"}')
tool_exec("gitlab_repo_clone", '{"project": "..."}')  # SKIP - clones repo
tool_exec("gitlab_issue_list", '{"project": "automation-analytics/automation-analytics-backend"}')
tool_exec("gitlab_issue_view", '{"project": "automation-analytics/automation-analytics-backend", "issue_id": 1}')
tool_exec("gitlab_issue_create", '{"project": "...", "title": "test"}')  # SKIP - creates
tool_exec("gitlab_label_list", '{"project": "automation-analytics/automation-analytics-backend"}')
tool_exec("gitlab_release_list", '{"project": "automation-analytics/automation-analytics-backend"}')
tool_exec("gitlab_user_info", '{}')
```

**Safe to test:** gitlab_mr_list, gitlab_mr_view, gitlab_mr_diff, gitlab_mr_approvers, gitlab_ci_list, gitlab_ci_status, gitlab_ci_view, gitlab_ci_trace, gitlab_ci_lint, gitlab_repo_view, gitlab_issue_list, gitlab_issue_view, gitlab_label_list, gitlab_release_list, gitlab_user_info (15 tools)
...
tool_exec("prometheus_targets", '{"environment": "stage"}')
tool_exec("prometheus_labels", '{"environment": "stage"}')
tool_exec("prometheus_series", '{"environment": "stage", "match": "up"}')
tool_exec("prometheus_namespace_metrics", '{"namespace": "tower-analytics-stage", "environment": "stage"}')
tool_exec("prometheus_error_rate", '{"namespace": "tower-analytics-stage", "environment": "stage"}')
...
**Safe to test:** ALL 11 tools (read-only)
...
```python
tool_exec("alertmanager_silences", '{"environment": "stage"}')
...
tool_exec("alertmanager_status", '{"environment": "stage"}')
tool_exec("alertmanager_alerts", '{"environment": "stage"}')
```

**Safe to test:** alertmanager_silences, alertmanager_status, alertmanager_alerts (3 tools)
...
tool_exec("kibana_get_errors", '{"namespace": "tower-analytics-stage", "environment": "stage", "limit": 2}')
tool_exec("kibana_get_pod_logs", '{"namespace": "tower-analytics-stage", "environment": "stage", "pod": "automation-analytics-api-fastapi-v2", "limit": 2}')
tool_exec("kibana_trace_request", '{"request_id": "xxx", "environment": "stage"}')
tool_exec("kibana_get_link", '{"namespace": "tower-analytics-stage", "environment": "stage"}')
tool_exec("kibana_error_link", '{"namespace": "tower-analytics-stage", "environment": "stage"}')
tool_exec("kibana_status", '{"environment": "stage"}')
tool_exec("kibana_index_patterns", '{"environment": "stage"}')
tool_exec("kibana_list_dashboards", '{"environment": "stage"}')
```

**Safe to test:** ALL 9 tools (read-only)
...
tool_exec("konflux_get_application", '{"application": "aap-aa-main", "namespace": "aap-aa-tenant"}')
...
tool_exec("konflux_list_release_plans", '{"application": "aap-aa-main", "namespace": "aap-aa-tenant"}')
...
tool_exec("konflux_list_environments", '{"namespace": "aap-aa-tenant"}')
tool_exec("konflux_namespace_summary", '{"namespace": "aap-aa-tenant"}')
```

**Safe to test:** ALL 15 tools (read-only)
...
tool_exec("bonfire_deploy_with_reserve", '{"app": "tower-analytics"}')  # SKIP - deploys
tool_exec("bonfire_process", '{"app": "tower-analytics"}')
tool_exec("bonfire_deploy_env", '{"namespace": "ephemeral-xxx"}')  # SKIP - deploys
tool_exec("bonfire_process_env", '{}')
tool_exec("bonfire_deploy_iqe_cji", '{"namespace": "ephemeral-xxx"}')  # SKIP - deploys
tool_exec("bonfire_pool_list", '{}')
tool_exec("bonfire_deploy_aa", '{"namespace": "ephemeral-xxx"}')  # SKIP - deploys
```

**Safe to test:** bonfire_version, bonfire_namespace_list, bonfire_namespace_describe, bonfire_namespace_wait, bonfire_apps_list, bonfire_apps_dependencies, bonfire_process, bonfire_process_env, bonfire_pool_list (9 tools)
**Skip (modifies state):** 8 tools
...
```python
tool_exec("quay_get_repository", '{"repository": "redhat-user-workloads/aap-aa-tenant/aap-aa-main/automation-analytics-backend-main"}')
tool_exec("quay_list_tags", '{"repository": "redhat-user-workloads/aap-aa-tenant/aap-aa-main/automation-analytics-backend-main", "limit": 5}')
...
**Safe to test:** ALL 8 tools (read-only)
...
```python
tool_exec("slack_list_messages", '{"channel": "automation-analytics-dev", "limit": 3}')
tool_exec("slack_send_message", '{"channel": "...", "message": "test"}')  # SKIP - sends
tool_exec("slack_get_channels", '{}')
...
tool_exec("slack_add_reaction", '{"channel": "...", "timestamp": "...", "emoji": "thumbsup"}')  # SKIP - modifies
tool_exec("slack_list_channels", '{}')
```

**Safe to test:** slack_list_messages, slack_get_channels, slack_get_user, slack_search_messages, slack_list_channels (5 tools)
**Skip (modifies state):** 4 tools
...
tool_exec("debug_tool", '{"tool_name": "git_status"}')
```

**Safe to test:** memory_read, memory_session_log, check_known_issues, tool_list, tool_exec, persona_list, session_start, skill_list, skill_run, debug_tool (10 tools)
**Skip (modifies state):** 7 tools

---

## Summary

| Module | Total | Safe to Test | Skip (Modifies) |
|--------|-------|--------------|-----------------|
| git | 15 | 8 | 7 |
| jira | 25 | 8 | 17 |
| gitlab | 30 | 15 | 15 |
