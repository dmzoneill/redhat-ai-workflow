...
class TestEnsureExportDir:
    ...
    @patch("tool_modules.aa_workflow.src.workspace_exporter.EXPORT_DIR")
...
class TestReadExportedState:
    ...
    @patch("tool_modules.aa_workflow.src.workspace_exporter.EXPORT_FILE")
    ...
    @patch("tool_modules.aa_workflow.src.workspace_exporter.EXPORT_FILE")
    def test_reads_valid_json(self, mock_file):
    ...
    @patch("tool_modules.aa_workflow.src.workspace_exporter.EXPORT_FILE")
    def test_handles_json_error(self, mock_file):
        mock_file.exists.return_value = True
...
class TestClearExportedState:
    ...
    @patch("tool_modules.aa_workflow.src.workspace_exporter.EXPORT_FILE")
    def test_clears_existing_file(self, mock_file):
        mock_file.exists.return_value = True
    ...
    @patch("tool_modules.aa_workflow.src.workspace_exporter.EXPORT_FILE")
    def test_clears_nonexistent_file(self, mock_file):
        mock_file.exists.return_value = False
        result = clear_exported_state()
        assert result is True
    ...
    @patch("tool_modules.aa_workflow.src.workspace_exporter.EXPORT_FILE")
    def test_handles_unlink_error(self, mock_file):
        mock_file.exists.return_value = True
...
class TestExportWorkspaceState:
    ...
    @patch("tool_modules.aa_workflow.src.workspace_exporter.EXPORT_FILE")
    def test_preserves_existing_data(self, mock_file, mock_export_with_data):
        mock_file.exists.return_value = True
        existing = {
        ...
        with patch("builtins.open", m):
            ...
            assert call_kwargs.kwargs.get("performance") == existing["performance"]

    @patch(
        "tool_modules.aa_workflow.src.workspace_exporter.export_workspace_state_with_data"
    )
    @patch("tool_modules.aa_workflow.src.workspace_exporter.EXPORT_FILE")
    def test_no_existing_file(self, mock_file, mock_export_with_data):
        mock_file.exists.return_value = False
        mock_export_with_data.return_value = {"success": True}
        result = export_workspace_state()
    ...
    @patch("tool_modules.aa_workflow.src.workspace_exporter.EXPORT_FILE")
    def test_existing_file_bad_json(self, mock_file, mock_export_with_data):
        mock_file.exists.return_value = True
...
class TestExportWorkspaceStateWithData:
    ...
    @patch("tool_modules.aa_workflow.src.workspace_exporter.EXPORT_FILE")
    @patch("tool_modules.aa_workflow.src.workspace_exporter.EXPORT_DIR")
    @patch("server.workspace_state.WorkspaceRegistry")
    def test_successful_export(self, mock_registry, mock_dir, mock_file):
        ...
        mock_registry.get_all_sessions.return_value = [{"id": "s1", "name": "session1"}]
        mock_file.exists.return_value = False
        mock_dir.mkdir = MagicMock()

        with (
            patch("tempfile.mkstemp", return_value=(99, "/tmp/test_tmp")),
            patch(
                "os.fdopen",
    ...
    @patch("tool_modules.aa_workflow.src.workspace_exporter.EXPORT_DIR")
    @patch("server.workspace_state.WorkspaceRegistry")
    def test_preserves_workspaces_when_registry_empty(
        self, mock_registry, mock_dir, mock_file
    ):
        mock_registry.sync_all_with_cursor.return_value = {
            "added": 0,
            "removed": 0,
    ...
    @patch("tool_modules.aa_workflow.src.workspace_exporter.EXPORT_DIR")
    @patch("server.workspace_state.WorkspaceRegistry")
    def test_cleanup_stale(self, mock_registry, mock_dir, mock_file):
        mock_registry.sync_all_with_cursor.return_value = {
            "added": 0,
            "removed": 0,
            "renamed": 0,
        ...
        mock_registry.get_all_as_dict.return_value = {"ws1": {"sessions": {}}}
        mock_registry.get_all_sessions.return_value = []
        mock_file.exists.return_value = False
        mock_dir.mkdir = MagicMock()
    ...
    @patch("tool_modules.aa_workflow.src.workspace_exporter.EXPORT_DIR")
    @patch("server.workspace_state.WorkspaceRegistry")
    def test_write_failure(self, mock_registry, mock_dir, mock_file):
        mock_registry.sync_all_with_cursor.return_value = {
            "added": 0,
            "removed": 0,
            "renamed": 0,
...
class TestExportWorkspaceStateAsync:
    ...
    @pytest.mark.asyncio
    @patch("tool_modules.aa_workflow.src.workspace_exporter.export_workspace_state")
    @patch("server.workspace_state.WorkspaceRegistry")
    async def test_without_ctx(self, mock_registry, mock_export):
        mock_registry.get_all_as_dict.return_value = {}
    ...
    @patch("tool_modules.aa_workflow.src.workspace_exporter.export_workspace_state")
    @patch("server.workspace_state.WorkspaceRegistry")
    async def test_with_ctx(self, mock_registry, mock_export):
        ctx = MagicMock()
        state = MagicMock()
        ...
        mock_export.return_value = {"success": True, "workspace_count": 1}
...
class TestOnWorkspaceChange:
    ...
    @patch("tool_modules.aa_workflow.src.workspace_exporter.export_workspace_state")
    def test_calls_export(self, mock_export):
        from tool_modules.aa_workflow.src.workspace_exporter import _on_workspace_change
