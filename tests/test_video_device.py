...
class TestFindExistingDevice:
    def test_found_ai_workflow(self):
    ...
    def test_found_ai_research(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
    ...
    def test_v4l2ctl_not_found(self):
        with patch("subprocess.run") as mock_run:
    ...
    def test_v4l2ctl_error_return(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(returncode=1, stdout="")
            assert find_existing_device() is None

    def test_device_label_but_no_device_path(self):
        with patch("subprocess.run") as mock_run:
    ...
    def test_label_at_last_line(self):
        with patch("subprocess.run") as mock_run:
...
class TestFindFreeDeviceNumber:
    def test_first_preferred(self):
        with patch("os.path.exists", return_value=False):
            assert find_free_device_number() == 10

    def test_third_preferred(self):
...
class TestGetDeviceFormat:
    ...
    def test_no_width_height_line(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="Format Video Capture:\n\tPixel Format : 'YU12'\n",
...
class TestSetupV4l2Device:
    def test_reuse_existing_matching_format(self):
        with (
            patch(
                "scripts.common.video_device.get_device_format",
            ...
            mock_run.return_value = MagicMock(
                returncode=0, stdout="v4l2loopback something"
            )
            result = setup_v4l2_device(1920, 1080)
            assert result == "/dev/video10"
            assert vd._active_device_path == "/dev/video10"

    def test_reload_on_format_mismatch(self):
        with (
            patch("scripts.common.video_device.get_device_format") as mock_fmt,
            patch(
                "scripts.common.video_device.find_existing_device",
                return_value=("/dev/video10", 10),
            ),
    ...
    def test_create_new_device(self):
        with (
            ...
            patch(
                "scripts.common.video_device.find_existing_device", return_value=None
            ),
            patch(
                "scripts.common.video_device.find_free_device_number", return_value=10
            ),
            patch("subprocess.run") as mock_run,
            patch("os.path.exists", return_value=True),
            patch("os.open", return_value=5),
    ...
    def test_device_not_appear_raises(self):
        with (
            patch(
                "scripts.common.video_device.find_existing_device", return_value=None
            ),
    ...
    def test_cannot_open_device_raises(self):
        with (
            patch(
                "scripts.common.video_device.get_device_format",
                return_value=(1920, 1080),
    ...
    def test_device_not_exists_after_setup_raises(self):
        with (
            patch(
                "scripts.common.video_device.get_device_format",
                return_value=(1920, 1080),
    ...
    def test_modprobe_fails_raises(self):
        with (
            patch(
                "scripts.common.video_device.find_existing_device", return_value=None
            ),
    ...
    def test_format_mismatch_warning(self):
        with (
            patch(
                "scripts.common.video_device.get_device_format", return_value=(640, 480)
            ) as mock_fmt,
            patch(
                "scripts.common.video_device.find_existing_device",
                return_value=("/dev/video10", 10),
...
class TestUnloadV4l2loopback:
    def test_module_not_loaded(self):
        with (
            ...
            mock_run.return_value = MagicMock(
                returncode=0, stdout="some_module 12345 0"
            )
    ...
    def test_kills_processes_on_devices(self):
        with (
            patch("subprocess.run") as mock_run,
            patch("os.path.exists") as mock_exists,
            patch("time.sleep"),
            ...
            fuser_calls = [
                ...
                if len(c[0]) > 0 and "fuser" in c[0][0]
            ]
