...
def register_tools(server: "FastMCP") -> int:
    ...
    async def konflux_running_pipelines(namespace: str = DEFAULT_NAMESPACE) -> str:
        ...
        success, output = await run_cmd([
            "kubectl", "get", "pipelineruns", "-n", namespace,
            "--field-selector=status.conditions[0].reason=Running", "-o", "wide"
        ])
        if not success:
    ...
    async def konflux_list_snapshots(namespace: str = DEFAULT_NAMESPACE, limit: int = 10) -> str:
        ...
        success, output = await run_cmd([
            "kubectl", "get", "snapshots", "-n", namespace, "-o", "wide",
    ...
    async def konflux_get_test_results(namespace: str = DEFAULT_NAMESPACE, snapshot: str = "", limit: int = 10) -> str:
        ...
        args = ["kubectl", "get", "pipelineruns", "-n", namespace,
                "-l", "test.appstudio.openshift.io/scenario",
                "-o", "wide", "--sort-by=.metadata.creationTimestamp"]
        if snapshot:
            args = ["kubectl", "get", "pipelineruns", "-n", namespace,
                    "-l", f"appstudio.openshift.io/snapshot={snapshot}",
                    "-o", "wide"]
        success, output = await run_cmd(args)
        if not success:
            return f"❌ Failed: {output}"
    ...
    @server.tool()
    async def tkn_pipelinerun_logs(run_name: str, namespace: str = DEFAULT_NAMESPACE, task: str = "", all_tasks: bool = True) -> str:
