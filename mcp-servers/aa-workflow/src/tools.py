...
- `e` or `ephemeral` = Ephemeral cluster""")]
    ...
    async def lint_yaml(
        ...
        success, stdout, stderr = await run_cmd(["yamllint", "-f", "parsable", target], cwd=repo_path)
    ...
    async def workflow_run_local_checks(
        ...
        if (Path(path) / "pyproject.toml").exists():
            # Python project
            for cmd_name, cmd in [("black --check", ["black", "--check", "."]),
    ...
    class SkillExecutor:
        ...
        async def _exec_tool(self, tool_name: str, args: dict) -> dict:
            ...
            module_prefixes = {
                "git_": "git", "jira_": "jira", "gitlab_": "gitlab",
                "kubectl_": "k8s", "k8s_": "k8s", "prometheus_": "prometheus",
                "alertmanager_": "alertmanager", "kibana_": "kibana",
                "konflux_": "konflux", "tkn_": "konflux", "bonfire_": "bonfire",
                "quay_": "quay", "appinterface_": "appinterface",
                # Workflow tools (various prefixes)
                "workflow_": "workflow", "lint_": "workflow", "test_": "workflow",
                "security_": "workflow", "precommit_": "workflow",
                "memory_": "workflow", "agent_": "workflow", "skill_": "workflow",
                "session_": "workflow", "tool_": "workflow",
            }
                ...
                if "tool" in step:
                    ...
                    if result["success"]:
                        ...
                        if on_error == "continue":
                            output_lines.append(f"   *Continuing despite error (on_error: continue)*\n")
                            self.step_results.append({
                                "step": step_name,
                                "tool": tool,
                                "success": False,
            ...
            try:
                ...
                if active or mrs or followups:
                    ...
                    if followups:
                        ...
                        for fu in followups:
                            lines.append(f"- {fu.get('task', '?')} (priority: {fu.get('priority', 'normal')})")
                        lines.append("")
                else:
    ...
    async def tool_list(module: str = "") -> list[TextContent]:
        ...
        tool_counts = {
            "git": ["git_status", "git_branch_list", "git_branch_create", "git_checkout", 
                    "git_log", "git_diff", "git_add", "git_commit", "git_push", "git_pull",
                    "git_stash", "git_fetch", "git_merge", "git_rebase", "git_remote"],
            "jira": ["jira_view_issue", "jira_view_issue_json", "jira_search", "jira_list_issues",
                     "jira_my_issues", "jira_list_blocked", "jira_lint", "jira_set_status",
                     "jira_assign", "jira_unassign", "jira_add_comment", "jira_block",
                     "jira_unblock", "jira_add_to_sprint", "jira_remove_sprint", "jira_create_issue",
                     "jira_clone_issue", "jira_add_link", "jira_add_flag", "jira_remove_flag",
                     "jira_open_browser"],
            "gitlab": ["gitlab_mr_list", "gitlab_mr_view", "gitlab_mr_create", "gitlab_mr_update",
                       "gitlab_mr_approve", "gitlab_mr_revoke", "gitlab_mr_merge", "gitlab_mr_close",
                       "gitlab_mr_reopen", "gitlab_mr_comment", "gitlab_mr_diff", "gitlab_mr_rebase",
                       "gitlab_mr_checkout", "gitlab_mr_approvers", "gitlab_ci_list", "gitlab_ci_status",
                       "gitlab_ci_view", "gitlab_ci_run", "gitlab_ci_retry", "gitlab_ci_cancel",
                       "gitlab_ci_trace", "gitlab_ci_lint", "gitlab_repo_view", "gitlab_repo_clone",
                       "gitlab_issue_list", "gitlab_issue_view", "gitlab_issue_create",
                       "gitlab_label_list", "gitlab_release_list", "gitlab_user_info"],
            "k8s": ["kubectl_get_pods", "kubectl_describe_pod", "kubectl_logs", "kubectl_delete_pod",
                    "kubectl_get_deployments", "kubectl_describe_deployment", "kubectl_rollout_status",
                    "kubectl_rollout_restart", "kubectl_scale", "kubectl_get_services",
                    "kubectl_get_events", "kubectl_get", "kubectl_exec", "kubectl_top_pods"],
            "prometheus": ["prometheus_query", "prometheus_query_range", "prometheus_alerts",
                          "prometheus_rules", "prometheus_targets", "prometheus_labels",
                          "prometheus_series", "prometheus_namespace_metrics", "prometheus_error_rate",
                          "prometheus_pod_health", "prometheus_grafana_link"],
            "alertmanager": ["alertmanager_silences", "alertmanager_create_silence",
                            "alertmanager_delete_silence", "alertmanager_status", "alertmanager_alerts"],
            "kibana": ["kibana_search_logs", "kibana_get_errors", "kibana_get_pod_logs",
                      "kibana_trace_request", "kibana_get_link", "kibana_error_link",
                      "kibana_status", "kibana_index_patterns", "kibana_list_dashboards"],
            "konflux": ["konflux_list_applications", "konflux_get_application", "konflux_list_components",
                       "konflux_get_component", "konflux_list_snapshots", "konflux_get_snapshot",
                       "konflux_list_integration_tests", "konflux_get_test_results", "konflux_list_releases",
                       "konflux_get_release", "konflux_list_release_plans", "konflux_list_builds",
                       "konflux_get_build_logs", "konflux_list_environments", "konflux_namespace_summary"],
            "bonfire": ["bonfire_version", "bonfire_namespace_reserve", "bonfire_namespace_list",
                       "bonfire_namespace_describe", "bonfire_namespace_release", "bonfire_namespace_extend",
                       "bonfire_namespace_wait", "bonfire_apps_list", "bonfire_apps_dependencies",
                       "bonfire_deploy", "bonfire_deploy_with_reserve", "bonfire_process",
                       "bonfire_deploy_env", "bonfire_process_env", "bonfire_deploy_iqe_cji",
                       "bonfire_pool_list", "bonfire_deploy_aa"],
            "quay": ["quay_get_repository", "quay_list_tags", "quay_get_tag", "quay_check_image_exists",
                    "quay_get_vulnerabilities", "quay_get_manifest", "quay_check_aa_image", "quay_list_aa_tags"],
            "appinterface": ["appinterface_validate", "appinterface_get_saas", "appinterface_diff",
                            "appinterface_resources", "appinterface_search", "appinterface_clusters"],
        }
