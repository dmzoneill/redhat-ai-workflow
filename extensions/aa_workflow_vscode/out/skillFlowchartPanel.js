...
exports.SkillFlowchartPanel = void 0;
exports.getSkillFlowchartPanel = getSkillFlowchartPanel;
exports.registerSkillFlowchartPanel = registerSkillFlowchartPanel;
...
class SkillFlowchartPanel {
    ...
    _execution;
    _availableSkills = [];
    static createOrShow(extensionUri, position = "bottom") {
        ...
        const viewColumn = position === "bottom" ? vscode.ViewColumn.Beside : vscode.ViewColumn.Two;
        // Create panel
        const panel = vscode.window.createWebviewPanel("aaSkillFlowchart", "⚡ Skill Flowchart", {
            viewColumn,
            preserveFocus: true,
        }, {
            enableScripts: true,
     ...
     */
    startExecution(skillName, inputs) {
    ...
    _startExecutionInternal(inputs) {
        if (!this._execution)
            return;
     ...
     */
    recordMemoryOperation(stepIndex, operation, key) {
        if (!this._execution || stepIndex >= this._execution.skill.steps.length) {
            return;
        }
        const step = this._execution.skill.steps[stepIndex];
     ...
     */
    recordHealing(stepIndex, details) {
        if (!this._execution || stepIndex >= this._execution.skill.steps.length) {
            return;
        }
        const step = this._execution.skill.steps[stepIndex];
    ...
    async _parseSkillYaml(skillName) {
        const skillsDir = path.join(os.homedir(), "src", "redhat-ai-workflow", "skills");
    ...
    _parseYamlContent(content, skillName) {
        ...
        for (const line of lines) {
            ...
            if (currentSection === "steps") {
                ...
                if (stepMatch) {
                    ...
                    if (descMatch)
                        currentStep.description = descMatch[1];
                    if (toolMatch)
                        currentStep.tool = toolMatch[1].trim();
                    if (conditionMatch)
                        currentStep.condition = conditionMatch[1];
                    if (outputMatch)
                        currentStep.output = outputMatch[1].trim();
                    if (onErrorMatch)
                        currentStep.onError = onErrorMatch[1].trim();
            ...
            if (currentSection === "inputs") {
                ...
                if (inputMatch) {
                    ...
                    const descMatch = line.match(/^\s+description:\s*["']?(.+?)["']?\s*$/);
                    if (typeMatch)
                        currentInput.type = typeMatch[1].trim();
                    if (requiredMatch)
                        currentInput.required = requiredMatch[1].trim();
                    if (defaultMatch)
                        currentInput.default = defaultMatch[1].trim();
                    if (descMatch)
     ...
     */
    _analyzeStepLifecycle(step, args) {
        ...
        const isAutoRemediation = autoRemediationPatterns.some((p) => name.toLowerCase().includes(p)) ||
    ...
    async _runSkill(skillName, inputs) {
        ...
        if (!success) {
            const selection = await vscode.window.showInformationMessage(`📋 Copied to clipboard: ${command}`, "Open New Chat", "Paste (Ctrl+V)");
    ...
    _getExecutionHtml(exec) {
        const skill = exec.skill;
        const completedSteps = skill.steps.filter((s) => s.status === "success" || s.status === "skipped").length;
    ...
    _getHorizontalStepHtml(step, index, isLastInRow = false) {
        ...
        if (step.compute)
            typeTags.push(`<span class="tag compute" title="Python compute">🐍</span>`);
        // Build lifecycle indicators
        const lifecycleIndicators = [];
        // Memory read indicator
        if (step.memoryRead && step.memoryRead.length > 0) {
            const memKeys = step.memoryRead.join(", ");
            lifecycleIndicators.push(`<span class="lifecycle-indicator memory-read" title="Memory Read: ${memKeys}">📖</span>`);
        }
        // Memory write indicator
        if (step.memoryWrite && step.memoryWrite.length > 0) {
            const memKeys = step.memoryWrite.join(", ");
            lifecycleIndicators.push(`<span class="lifecycle-indicator memory-write" title="Memory Write: ${memKeys}">💾</span>`);
        }
        // Auto-remediation indicator
        if (step.isAutoRemediation) {
            lifecycleIndicators.push(`<span class="lifecycle-indicator auto-heal" title="Auto-remediation step">🔄</span>`);
        }
        // Can retry indicator
        if (step.canRetry && !step.isAutoRemediation) {
            lifecycleIndicators.push(`<span class="lifecycle-indicator can-retry" title="Can retry on error">↩️</span>`);
        }
        // Healing applied indicator (runtime)
        if (step.healingApplied) {
            lifecycleIndicators.push(`<span class="lifecycle-indicator healed" title="Auto-heal applied: ${step.healingDetails || 'Fixed'}">✨</span>`);
        }
        // Retry count indicator (runtime)
        if (step.retryCount && step.retryCount > 0) {
            lifecycleIndicators.push(`<span class="lifecycle-indicator retry-count" title="Retried ${step.retryCount} time(s)">🔁${step.retryCount}</span>`);
        }
        ...
        if (step.memoryRead?.length)
            tooltipParts.push(`Reads: ${step.memoryRead.join(", ")}`);
        if (step.memoryWrite?.length)
            tooltipParts.push(`Writes: ${step.memoryWrite.join(", ")}`);
        if (step.isAutoRemediation)
            tooltipParts.push("Auto-remediation step");
    ...
    _getStepHtml(step, index) {
        ...
        if (step.compute)
            tags.push(`<span class="step-tag compute">🐍 compute</span>`);
        if (step.condition)
            tags.push(`<span class="step-tag condition">❓ conditional</span>`);
        // Lifecycle tags
        if (step.memoryRead && step.memoryRead.length > 0) {
    ...
    _getStepIcon(status, stepNumber) {
        switch (status) {
            case "success":
                return "✓";
            case "failed":
...
exports.SkillFlowchartPanel = SkillFlowchartPanel;
// ============================================================================
// Utilities
// ============================================================================
function getNonce() {
 ...
 */
function getSkillFlowchartPanel() {
    return SkillFlowchartPanel.currentPanel;
}
function registerSkillFlowchartPanel(context) {
    // Command to open flowchart panel
    context.subscriptions.push(vscode.commands.registerCommand("aa-workflow.openSkillFlowchart", () => {
        SkillFlowchartPanel.createOrShow(context.extensionUri);
    }));
    // Command to visualize a specific skill
    context.subscriptions.push(vscode.commands.registerCommand("aa-workflow.visualizeSkillFlowchart", async (skillName) => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("aa-workflow.showSkillExecution", (skillName, inputs) => {
        const panel = SkillFlowchartPanel.createOrShow(context.extensionUri);
        panel.startExecution(skillName, inputs);
    }));
}
//# sourceMappingURL=skillFlowchartPanel.js.map
