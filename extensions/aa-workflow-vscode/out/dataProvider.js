...
exports.WorkflowDataProvider = void 0;
...
class WorkflowDataProvider {
    ...
    constructor() {
        this.memoryDir = path.join(os.homedir(), ".config", "aa-workflow", "memory");
        this.configPath = path.join(os.homedir(), "src", "redhat-ai-workflow", "config.json");
    ...
    async refreshEnvironment() {
        try {
            ...
            const prodMatch = content.match(/production:\s*\n(?:.*\n)*?\s*status:\s*(\w+)/);
            const stageAlertsMatch = content.match(/stage:\s*\n(?:.*\n)*?\s*alerts:\s*\[(.*?)\]/s);
            const prodAlertsMatch = content.match(/production:\s*\n(?:.*\n)*?\s*alerts:\s*\[(.*?)\]/s);
     ...
     */
    parseYamlList(content, listName) {
        const items = [];
...
exports.WorkflowDataProvider = WorkflowDataProvider;
