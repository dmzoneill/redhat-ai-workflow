...
class TextWakeWordDetector:
    ...
    def __init__(self, wake_word: str = "david"):
        ...
        # Timing
        self.pause_threshold = timedelta(seconds=2)  # Pause after wake word = command complete
        self.command_timeout = timedelta(seconds=10)  # Max time to wait for command
    ...
    def process_caption(self, speaker: str, text: str) -> Optional[WakeWordEvent]:
        ...
        if self.pattern.search(text):
            ...
            if match:
                ...
                # If there's already text after the wake word, it might be the complete command
                if command_start and len(command_start.split()) >= 3:
                    # Likely a complete command in one caption
...
class WakeWordManager:
    ...
    Uses text-based detection as primary (from captions),
