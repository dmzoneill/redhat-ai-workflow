...
from tool_modules.aa_meet_bot.src.config import get_config, MeetBotConfig
from tool_modules.aa_meet_bot.src.jira_preloader import JiraPreloader, get_jira_preloader, preload_jira_context
from tool_modules.aa_meet_bot.src.llm_responder import LLMResponder, get_llm_responder, generate_meeting_response
from tool_modules.aa_meet_bot.src.tts_engine import GPTSoVITSEngine, get_tts_engine, TTSResult
from tool_modules.aa_meet_bot.src.video_generator import VideoGenerator, get_video_generator, VideoResult
...
from tool_modules.aa_meet_bot.src.notes_bot import NotesBot, get_notes_bot, init_notes_bot, NotesBotManager, get_bot_manager
...
async def _meet_bot_join_meeting_impl(
    ...
    def on_caption(entry: CaptionEntry):
        ...
        if event:
            logger.info(f"Wake word event: {event}")
        ...
        for status in statuses:
            if status.get('status') == 'capturing':
                ...
                current_meetings.append({
                    ...
                    "startTime": status.get('joined_at', datetime.now()).isoformat() if isinstance(status.get('joined_at'), datetime) else datetime.now().isoformat(),
                    "organizer": "",
                    "attendees": [],
                    "status": "joined",
        ...
        "recentNotes": [
            {
                ...
                "duration": int((m.actual_end - m.actual_start).total_seconds() / 60) if m.actual_start and m.actual_end else 0,
