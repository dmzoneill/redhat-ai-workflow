...
class NotesBot:
    ...
    def _on_caption(self, entry: CaptionEntry) -> None:
        ...
        if cap_id and cap_id in self.state.caption_id_to_index:
            ...
            if 0 <= idx < len(self.state.transcript_buffer):
                ...
                self.state.caption_id_to_index[cap_id] = len(
            ...
            if last_entry.speaker == entry.speaker:
                ...
                if cap_id:
                    ...
                    self.state.caption_id_to_index[cap_id] = (
                        len(self.state.transcript_buffer) - 1
                    )
                logger.debug(
    ...
    async def _setup_interactive_mode(self) -> None:
        ...
        try:
            ...
            meeting_id = (
                str(self.state.meeting_id)
                if self.state.meeting_id
                else f"meet-{id(self)}"
            )
...
from .notes_bot_manager import (  # noqa: F401, E402
    MeetingSession,
    NotesBotManager,
    get_bot_manager,
    get_notes_bot,
