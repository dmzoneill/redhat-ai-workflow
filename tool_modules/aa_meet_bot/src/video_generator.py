...
from .video_overlay import VideoOverlayRenderer  # noqa: E402
from .video_streaming import V4L2OutputStream  # noqa: E402
...
class RealtimeVideoRenderer:
    ...
    def __init__(
        ...
        # Overlay renderer (text, waveform, NPU panel, layout composition)
        self._overlay = VideoOverlayRenderer(self.config, self._gpu_text_renderer)
    ...
    # Static cache is now managed by self._overlay (VideoOverlayRenderer)
    ...
    def _compute_fft_bars(self, audio: np.ndarray) -> np.ndarray:
        ...
        Delegates to VideoOverlayRenderer.compute_fft_bars.
        """
        return self._overlay.compute_fft_bars(audio)
    ...
    def _generate_waveform_frame_from_audio(
        ...
        Delegates to VideoOverlayRenderer.generate_waveform_frame_from_audio.
        """
        return self._overlay.generate_waveform_frame_from_audio(bar_heights, out)

    def _generate_waveform_frame(self, t: float, out: np.ndarray = None) -> np.ndarray:
        """Generate a single waveform frame at time t.

        Delegates to VideoOverlayRenderer.generate_waveform_frame.
        """
        return self._overlay.generate_waveform_frame(t, out)

    def _generate_npu_frame(
        self, t: float, frame_num: int, out: np.ndarray = None
    ) -> np.ndarray:
        """Generate a single NPU panel frame.

        Delegates to VideoOverlayRenderer.generate_npu_frame.
        """
        return self._overlay.generate_npu_frame(t, frame_num, out)
    ...
    async def stream_realtime(
        ...
        # Open v4l2 output stream (handles device setup and format negotiation)
        v4l2_stream = V4L2OutputStream(video_device, config.width, config.height)
        v4l2_stream.open()
    ...
    async def _stream_gpu_pipeline(
        ...
        from .video_overlay import FAKE_FINDINGS, RESEARCH_TOOLS, THREAT_ASSESSMENTS
        ...
        for frame_num in range(total_frames):
            ...
            # Write to v4l2
            if not v4l2_stream.write_frame(yuyv):
                return False
    ...
    def _create_base_frame(
        ...
        Delegates to VideoOverlayRenderer.create_base_frame with current
        NPU/STT stats from the renderer's monitoring loops.
        """
        return self._overlay.create_base_frame(
            attendee=attendee,
            attendee_idx=attendee_idx,
            total_attendees=total_attendees,
            tools=tools,
            findings=findings,
            ...
            npu_stats=self._npu_stats,
            stt_stats=self._stt_stats,
    ...
    def _precompute_attendee_data(self, attendee: Attendee) -> dict:
        ...
        Delegates to VideoOverlayRenderer.precompute_attendee_data.
        """
        return self._overlay.precompute_attendee_data(attendee)
    ...
    def _generate_complete_frame_rgb(
        ...
        Delegates to VideoOverlayRenderer.generate_complete_frame_rgb.
        """
        return self._overlay.generate_complete_frame_rgb(
            t=t,
            frame_num=frame_num,
            attendee=attendee,
            attendee_idx=attendee_idx,
