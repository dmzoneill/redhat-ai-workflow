...
async def _workflow_check_deploy_readiness_impl(
    project: str,
    mr_id: int,
    environment: str,
) -> list[TextContent]:
    """Implementation of workflow_check_deploy_readiness tool."""
...
async def _workflow_create_branch_impl(
    issue_key: str,
    repo: str,
    branch_type: str,
) -> list[TextContent]:
    """Implementation of workflow_create_branch tool."""
    try:
...
async def _workflow_daily_standup_impl(author: str) -> list[TextContent]:
    """Implementation of workflow_daily_standup tool."""
...
async def _workflow_handle_review_impl(
    issue_key: str,
    repo: str,
) -> list[TextContent]:
    """Implementation of workflow_handle_review tool."""
    try:
        path = resolve_path(repo)
    except ValueError as e:
        return [TextContent(type="text", text=f"❌ {e}")]
...
async def _workflow_monitor_pipelines_impl(
    repo: str,
    branch: str,
) -> list[TextContent]:
    """Implementation of workflow_monitor_pipelines tool."""
    try:
        path = resolve_path(repo)
    except ValueError as e:
        return [TextContent(type="text", text=f"❌ {e}")]
...
async def _workflow_prepare_mr_impl(
    issue_key: str,
    repo: str,
    draft: bool,
) -> list[TextContent]:
    """Implementation of workflow_prepare_mr tool."""
    try:
        path = resolve_path(repo)
    except ValueError as e:
        return [TextContent(type="text", text=f"❌ {e}")]
...
async def _workflow_review_feedback_impl(
    project: str,
    mr_id: int,
) -> list[TextContent]:
    """Implementation of workflow_review_feedback tool."""
...
async def _workflow_run_local_checks_impl(
    repo: str,
    skip_tests: bool,
) -> list[TextContent]:
    """Implementation of workflow_run_local_checks tool."""
    try:
        path = resolve_path(repo)
    except ValueError as e:
        return [TextContent(type="text", text=f"❌ {e}")]
...
async def _workflow_start_work_impl(issue_key: str) -> list[TextContent]:
    """Implementation of workflow_start_work tool."""
    issue_key = issue_key.upper()
...
"""
    ...
    async def workflow_daily_standup(
        ...
        """
        return await _workflow_daily_standup_impl(author)
    ...
    async def workflow_handle_review(
        ...
        """
        return await _workflow_handle_review_impl(issue_key, repo)
    ...
    async def workflow_monitor_pipelines(
        ...
        """
        return await _workflow_monitor_pipelines_impl(repo, branch)
    ...
    async def workflow_prepare_mr(
        ...
        """
        return await _workflow_prepare_mr_impl(issue_key, repo, draft)
    ...
    async def workflow_review_feedback(
        ...
        """
        return await _workflow_review_feedback_impl(project, mr_id)
    ...
    async def workflow_run_local_checks(
        ...
        Returns:
            Lint results.
        """
        return await _workflow_run_local_checks_impl(repo, skip_tests)
    ...
    async def workflow_start_work(issue_key: str) -> list[TextContent]:
        ...
        """
        return await _workflow_start_work_impl(issue_key)
