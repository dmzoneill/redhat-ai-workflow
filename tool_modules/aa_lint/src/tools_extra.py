...
def _get_resolve_path():
    """Create a resolve_path function with current config."""
    config = load_config()
...
async def _lint_dockerfile_impl(
    repo: str,
    ...
    """
    resolve_path = _get_resolve_path()
    try:
...
async def _security_scan_impl(repo: str) -> list[TextContent]:
    """
...
async def _test_coverage_impl(repo: str) -> list[TextContent]:
    """
...
def register_tools(server: "FastMCP") -> int:
    ...
    async def lint_dockerfile(
        ...
        Returns:
            Dockerfile best practice suggestions.
        """
        return await _lint_dockerfile_impl(repo, dockerfile)
    ...
    async def lint_yaml(
        ...
        Returns:
            YAML validation results.
        """
        return await _lint_yaml_impl(repo, path_filter)
    ...
    async def precommit_run(
        ...
        Returns:
            Pre-commit hook results.
        """
        return await _precommit_run_impl(repo, all_files)
    ...
    async def security_scan(repo: str) -> list[TextContent]:
        ...
        Returns:
            Security vulnerabilities found.
        """
        return await _security_scan_impl(repo)
    ...
    async def test_coverage(repo: str) -> list[TextContent]:
        ...
        Returns:
            Coverage percentage and uncovered files.
        """
        return await _test_coverage_impl(repo)
    ...
    async def test_run(
        ...
        Returns:
            Test results summary.
        """
        return await _test_run_impl(repo, test_path, verbose, coverage)
