...
async def run_cmd(args, cwd=None, timeout=60):
    """Run a command and return (success, stdout, stderr)."""
    return await run_cmd_full(args, cwd=cwd, timeout=timeout)


def register_lint_tools(server: "FastMCP") -> int:
    ...
    async def security_scan(repo: str) -> list[TextContent]:
        ...
        if (Path(repo_path) / "pyproject.toml").exists() or (Path(repo_path) / "requirements.txt").exists():
            ...
            success, stdout, stderr = await run_cmd(["bandit", "-r", ".", "-f", "txt", "-ll"], cwd=repo_path)
