...
def _get_remediation_hints(error_text: str, tool_name: str) -> list[str]:
    ...
    # VPN connectivity issues (patterns from server.error_patterns)
    if any(pattern in error_lower for pattern in VPN_PATTERNS):
        hints.append("🌐 VPN may be disconnected. Try: `vpn_connect()`")

    # Kubernetes auth issues (patterns from server.error_patterns)
    ...
    # GitLab auth issues (patterns from server.error_patterns)
    ...
    # Slack auth issues (patterns from server.error_patterns)
